      ...
      {entry && (
        <div
          ...
          style={{
            ...
            boxShadow: "0 18px 40px rgba(14,42,71,.24)",
          }}
        >
          <span style={{ flex: 1 }}>{entry.message}</span>
          <button type="button" onClick={() => setEntry(null)} aria-label="Dismiss" style={{ all: "unset", cursor: "pointer", color: "#fff", opacity: 0.8 }}>
            ✕
          </button>
        </div>
      )}
...
export function useFlash(): FlashContextValue {
  ...
  if (!ctx) throw new Error("useFlash must be used inside FlashProvider (SportsShell mounts it once)");
  return ctx;
}
