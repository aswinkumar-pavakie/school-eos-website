...
// Generic person search for granting a role assignment to any person in the
// school (staff or student) -- same real GET /persons?search= proxy
// GuardianPersonPicker already uses, just without its roleCode=PARENT filter,
// since a role assignment can target any person.
...
export function PersonPicker({ disabled }: { disabled?: boolean }) {
  ...
  return (
    <div className="relative flex flex-col gap-1.5 text-sm">
      <span className="font-semibold text-text">Person *</span>
      ...
      {!selected && <span className="text-xs text-text-muted">{loading ? "Searching…" : "Search for an existing person."}</span>}
