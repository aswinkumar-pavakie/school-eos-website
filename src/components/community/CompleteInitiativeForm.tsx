...
import { completeInitiativeAction } from "@/app/(dashboard)/community/activities/actions";
import type { FormActionState } from "@/app/(dashboard)/community/activities/actions";

const initialState: FormActionState = {};

export function CompleteInitiativeForm({ id }: { id: string }) {
  const [state, formAction, isPending] = useActionState(completeInitiativeAction, initialState);

  return (
    <form action={formAction} className="flex flex-col gap-3 rounded-[16px] border border-border bg-surface p-[18px]">
      <input type="hidden" name="id" value={id} />
      <h2 className="text-[15px] font-extrabold leading-[20px] text-text">Mark complete</h2>
      {state.error && (
