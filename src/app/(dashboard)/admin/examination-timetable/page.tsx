// Admin -- Exam timetable: per the SIS Principal mockup's own screen (nav
// item "Exam timetable"), pixel-matched the same way as the Principal/Vice
// Principal pages. Thin data-fetching wrapper around the shared
// ExamTimetableView (src/components/academics/ExamTimetableView.tsx), which
// also backs Principal and Vice Principal so all three render identically
// off the same real exam/exam_subject schedule (exams.controller.ts,
// granted to ADMIN) and marksEnteredCount (a real count of `mark` rows per
// paper -- see exam.repository.ts's own comment). Read-only here too: to add
// or edit subjects, Admin uses the separate Examinations module
// (/admin/examinations), linked from the result card's exam name in spirit
// (that page owns the create/edit/publish/lock actions, not this one).

import { ExamTimetableView, type ExamRow, type ExamScheduleRow } from "@/components/academics/ExamTimetableView";
import { apiFetch } from "@/lib/api";

export default async function AdminExamTimetablePage({
  searchParams,
}: {
  ...
  return (
    <ExamTimetableView
      formAction="/admin/examination-timetable"
