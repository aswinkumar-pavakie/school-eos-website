// Vice Principal -- Attendance Sessions: a REAL oversight module Principal's
// own web console does not have at all. Backend audit of
// attendance-sessions.controller.ts found its class-level stays ADMIN-only
// (PRINCIPAL was never broadened onto it, unlike students/staff/parents) --
// VICE_PRINCIPAL gets its own explicit method-level grant instead ("Phase 7
// mobile Attendance module"). Read-only here too: list/get only, matching the
...
export default async function VicePrincipalAttendanceSessionsPage({
  ...
  function hrefWith(overrides: Record<string, string | undefined>) {
    ...
    return `/vice-principal/attendance-sessions?${next.toString()}`;
  }
  ...
  return (
    <div className="mx-auto max-w-[1024px]">
      ...
      <form action="/vice-principal/attendance-sessions" className="mt-5 flex flex-wrap items-end gap-3">
              ...
              return (
                <tr key={s.id} className="card-hover">
                  <td className="px-4 py-3 font-semibold text-text">
                    <Link href={`/vice-principal/attendance-sessions/${s.id}`} className="hover:underline">
