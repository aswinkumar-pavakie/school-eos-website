...
// than a plain table (this page previously mirrored Principal's own
// byte-for-byte). That real data source has no separate ON_DUTY status or
// punch-in/out times (only CHECK_IN/ABSENT plus a single occurredAt
// timestamp), so those are mapped as faithfully as the real shape allows:
// CHECK_IN -> PRESENT for the month-grid's coloring, and occurredAt's
// time-of-day stands in for the punch line.
...
export default async function VicePrincipalMyAttendancePage({ searchParams }: { searchParams: Promise<{ month?: string }> }) {
  ...
  return (
    <MyAttendanceView
      basePath="/vice-principal/my-attendance"
