// Principal -> Inventory: read-only oversight of the same real assets/stock
// Admin's own Inventory list shows -- no create/add-stock/issue/etc. controls.
// Inventory operational management stays with Admin; Principal gets the same
// overview KPIs, filters and item list, view-only.
...
export default async function PrincipalInventoryPage({
  searchParams,
}: {
  ...
  function hrefWith(overrides: Record<string, string | undefined>) {
    ...
    return `/vice-principal/inventory?${next.toString()}`;
  }
  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      ...
      <InventoryFilterBar
        ...
        formActionOverride="/vice-principal/inventory"
      />
            ...
            {items.map((item) => (
              <tr key={item.id} className="card-hover">
                ...
                <td className="px-4 py-3 text-right">
                  <Link href={`/vice-principal/inventory/items/${item.id}`} className="text-[13px] font-semibold text-primary">
                    View
                  </Link>
                </td>
              </tr>
