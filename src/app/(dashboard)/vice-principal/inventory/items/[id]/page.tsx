// Vice Principal -> Inventory -> item detail: read-only mirror of Admin's own
// item detail page (details, status, assignment, and the repair requests
// raised against this item -- the real Inventory <-> Maintenance
// integration, reused as-is via GET /repair-requests?inventoryItemId=, not
// duplicated). No add-stock/issue/return/transfer/mark-damaged/mark-lost/
// retire controls -- those stay Admin operational actions. No "+ New
// request" link either -- creating a repair request stays an Admin
// operational action. No "View history" link -- unlike Principal, VP has no
// Audit Log access at all (same established restriction already applied on
// VP's own maintenance/[id] page), so this page has nowhere real to send
// that link.
...
export default async function PrincipalInventoryItemDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[860px]">
      <BackLink href="/vice-principal/inventory" label="Back to Inventory" />
      ...
      <section className="mt-6 rounded-[16px] border border-border bg-surface p-[18px]">
        ...
        <ul className="mt-3 flex flex-col divide-y divide-border">
          ...
          {repairRequests.map((r) => (
            <li key={r.id} className="card-hover flex items-center justify-between rounded-[10px] px-2 py-2.5 text-sm">
              <div>
