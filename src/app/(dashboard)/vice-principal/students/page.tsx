...
export default async function PrincipalStudentsPage({
  ...
  function hrefWith(overrides: Record<string, string | undefined>) {
    ...
    return `/vice-principal/students?${next.toString()}`;
  }
  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      ...
      <StudentsFilterBar
        ...
        formActionOverride="/vice-principal/students"
      />
            ...
            {students.map((student) => (
              <tr key={student.id} className="hover:bg-field">
                <td className="px-4 py-3 font-semibold text-text">
                  <Link href={`/vice-principal/students/${student.id}`} className="flex items-center gap-2.5 hover:text-primary hover:underline">
                    <PersonAvatar
