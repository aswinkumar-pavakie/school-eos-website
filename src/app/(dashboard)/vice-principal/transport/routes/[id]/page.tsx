// Vice Principal -> Transport -> Route detail: read-only oversight (Vehicle
// -> Route -> Driver, and Student -> Route -> Stop -> Vehicle). No
// add/change/cancel controls anywhere -- assigning a student to a
// route/stop, editing vehicle specs, or crew changes all stay Admin/
// Transport Manager actions.
//
// Rebuilt to pixel-match the SIS Principal mockup's Route detail screen via
// TransportOversightRouteDetail, the exact same shared component Principal's
// own route detail page renders -- see that component's own comment for the
// two fields (Term fee, Safety & fitment) honestly shown as not tracked/not
// recorded rather than fabricated to match the mockup.
...
export default async function VicePrincipalRouteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <TransportOversightRouteDetail
      basePath="/vice-principal/transport"
      route={route}
      stops={stops}
      students={students}
      vehicle={vehicle}
