// New route -- splits Vice Principal's own on-duty requests out of the
// folded my-leave list into their own screen, matching Faculty's own
// separate Leave/OD screens (staff-od/page.tsx) instead of one combined
// list. Same real backend as my-leave (staff_leave_request,
// leaveType='ON_DUTY') -- zero new backend, reuses listMyLeaveRequests/
// createMyLeaveAction/withdrawMyLeaveAction from my-leave/actions.ts
// unchanged. Renders the same shared src/components/shared-ui/LeaveOdView
// shell Leave uses.
...
export default async function VicePrincipalMyOdPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
