// Principal -> Communities: read-only oversight of the same real communities
// Admin's own Communities list shows -- no create control. Community
// operational management (creation, edits, membership, activities,
// announcements) stays with Admin; Principal gets visibility, not the write
// actions.
...
export default async function PrincipalCommunityPage({
  searchParams,
}: {
  ...
  function hrefWith(overrides: Record<string, string | undefined>) {
    ...
    return `/vice-principal/community?${next.toString()}`;
  }
  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      ...
      <form action="/vice-principal/community" className="mt-6 flex flex-wrap items-end gap-3">
            ...
            {communities.map((c) => (
              <tr key={c.id} className="card-hover">
                ...
                <td className="px-4 py-3 text-right">
                  <Link href={`/vice-principal/community/${c.id}`} className="text-[13px] font-semibold text-primary">
                    View
                  </Link>
                </td>
              </tr>
