// Principal's Parents list -- read-only oversight, reusing Admin's exact list
// design (search/filter, paginated table) and the same real GET /parents
// endpoint. Deliberately excludes Admin's write/operational controls (Create
// parent, bulk ID-card selection/print) -- Principal browses and views, never
// creates or bulk-operates on records.
...
export default async function PrincipalParentsPage({
  searchParams,
}: {
  ...
  function hrefWith(overrides: Record<string, string | undefined>) {
    ...
    return `/vice-principal/parents?${next.toString()}`;
  }
  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      ...
      <form action="/vice-principal/parents" className="mt-6 flex flex-wrap items-end gap-3">
            ...
            {parents.map((parent) => (
              <tr key={parent.id} className="hover:bg-field">
                <td className="px-4 py-3 font-semibold text-text">
                  <Link href={`/vice-principal/parents/${parent.id}`} className="flex items-center gap-2.5 hover:text-primary hover:underline">
                    <PersonAvatar
