// Principal -> Finance -> Payments: read-only mirror of Admin's own
// /admin/finance/payments page. Student links point at Principal's OWN
// Students module (/vice-principal/students/[id]), never Admin's.
...
export default async function PrincipalFinancePaymentsPage({
  searchParams,
}: {
  ...
  function hrefWith(overrides: Record<string, string | undefined>) {
    ...
    return `/vice-principal/finance/payments?${next.toString()}`;
  }
  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      ...
      <FinanceTabBar active="Payments" basePath="/vice-principal/finance" />

      <PaymentsFilterBar
        search={params.search ?? ""}
        state={params.state ?? ""}
        mode={params.mode ?? ""}
        formActionOverride="/vice-principal/finance/payments"
      />
            ...
            {payments.map((p) => (
              <tr key={p.id} className="card-hover">
                <td className="px-4 py-3 font-semibold text-text">
                  {p.studentId ? (
                    <Link href={`/vice-principal/students/${p.studentId}`} className="hover:underline">
