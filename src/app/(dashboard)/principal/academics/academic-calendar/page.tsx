// Principal -> Academic Calendar: read-only oversight of the same real
// calendar_event data Admin's own page shows (term dates, holidays, exam
// windows, PTMs, functions, competitions, working Saturdays). Deliberately no
// create/delete controls -- calendar creation/removal stays an Admin
// operational action; there is no approval workflow gating calendar events for
...
export default async function PrincipalAcademicCalendarPage({
  ...
  return (
    <div className="mx-auto max-w-[1100px]">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-text-muted">
            Term dates, holidays, exams, and school events — view-only. Adding or removing events is an Admin action.
          </p>
        </div>
      </div>

      <form action="/principal/academics/academic-calendar" className="mt-6 flex items-end gap-3">
      ...
      <MonthCalendar events={events} campuses={campuses} grades={grades} sections={sections} readOnly />
    </div>
  );
}
