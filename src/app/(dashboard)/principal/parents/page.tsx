...
export default async function PrincipalParentsPage({
  ...
  function hrefWith(overrides: Record<string, string | undefined>) {
    ...
    return `/principal/parents?${next.toString()}`;
  }
  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      ...
      <form action="/principal/parents" className="mt-6 flex flex-wrap items-end gap-3">
            ...
            {parents.map((parent) => (
              <tr key={parent.id} className="hover:bg-field">
                <td className="px-4 py-3 font-semibold text-text">
                  <Link href={`/principal/parents/${parent.id}`} className="flex items-center gap-2.5 hover:text-primary hover:underline">
                    <PersonAvatar
