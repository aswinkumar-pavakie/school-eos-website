// Principal -> Audit Log: same general, real audit trail Admin's own
// /admin/audit page reads (GET /audit-log), reused verbatim -- this page has
// no write UI anywhere (pure read/filter/paginate), so nothing needed
// stripping down. The `returnTo` guard (only trust an internal, root-relative
// path) stays as-is; Principal's own item-detail pages pass their own
// /principal/* return paths.
...
export default async function PrincipalAuditPage({
  searchParams,
}: {
  ...
  function hrefWith(overrides: Record<string, string | undefined>) {
    ...
    return `/principal/audit?${next.toString()}`;
  }
