// Community -> Communities: read-only view of the same real communities
// Admin's own Communities list shows (backend: src/modules/communities,
// already Admin-write/Principal-oversight -- COMMUNITY added to its
// class-level @Roles in Phase 3/4 of this separate initiative). No create
// control -- community operational management (creation, edits, membership,
// activities, announcements) stays with Admin; Community gets visibility,
// not the write actions. Route is /community/community (role namespace +
// feature name) -- looks redundant only because this role happens to share
// its name with the feature; same /{role}/community pattern Admin and
// Principal already use for this exact page.
...
export default async function CommunityCommunitiesPage({
  searchParams,
}: {
  ...
  function hrefWith(overrides: Record<string, string | undefined>) {
    ...
    return `/community/community?${next.toString()}`;
  }
  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      ...
      <form action="/community/community" className="mt-6 flex flex-wrap items-end gap-3">
            ...
            {communities.map((c) => (
              <tr key={c.id}>
                ...
                <td className="px-4 py-3 text-right">
                  <Link href={`/community/community/${c.id}`} className="text-[13px] font-semibold text-primary">
                    View
                  </Link>
                </td>
              </tr>
