// Route detail -- Transport Manager's own equivalent of Admin's route detail
// page, reachable via Routes' "View route" (routesBasePath). Read-only:
// RouteAssignedStudents' own readOnly prop hides "+ Add student"/"Change
// stop"/"Remove"/"View student" -- student_transport_allocation write access
// stays Admin-only (see student-transport-allocations.controller.ts).
...
export default async function TransportManagerRouteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[900px]">
      <BackLink href="/transport-manager/routes" label="Back to Routes" />
      ...
      <section className="mt-8 rounded-[16px] border border-border bg-surface p-[18px]">
        ...
        <p className="mt-2 text-xs text-text-muted">Assign a bus/driver from Bus Allocation.</p>
      </section>
      ...
      <section className="mt-6 rounded-[16px] border border-border bg-surface p-[18px]">
        ...
        <p className="mt-1 text-[13px] text-text-muted">Who is riding this route, and at which stop — view only.</p>
        <div className="mt-3">
          <RouteAssignedStudents routeId={route.id} stops={sortedStops} students={students} readOnly />
        </div>
      </section>
    </div>
  );
