...
import { StageForms } from "../purchase-requests/[id]/StageForms";
import { ApprovalStatusPanel } from "./ApprovalStatusPanel";

// Shared by Finance's own /finance/purchase-requests/[id] and Principal's
// /principal/finance/purchase-requests/[id] (a Principal-shelled read-only view,
// reached from "View underlying record" on an approval) -- same data, same
// component, so there is exactly one place this page's logic lives. Write actions
// (StageForms: update stage, allot) only ever render for isFinanceOrAdmin, which a
// Principal caller never is -- Principal already sees a read-only view of this
// exact same component with zero extra code, matching "Principal doesn't perform
// the underlying purchase/service operation."
export async function PurchaseRequestDetailView({
  id,
  backHrefOverride,
  backLabelOverride,
  decideHrefBuilder,
  ...
  id: string;
  /** Overrides the default Finance-shaped back-link, for a caller (e.g. Principal's
   * own wrapper) that isn't reached from Finance's own tracking/approval queues. */
  backHrefOverride?: string;
  backLabelOverride?: string;
  /** Overrides ApprovalStatusPanel's default /finance/approvals/{id} "Decide /
   * manage" link, for the same reason. */
  ...
  try {
    ...
    const backHref =
      backHrefOverride ??
      (!isFinanceOrAdmin ? "/finance/purchase-requests" : order ? `/finance/${kind}-tracking` : `/finance/${kind}-approval`);
    const backLabel =
      backLabelOverride ??
      (!isFinanceOrAdmin ? "My Requests" : order ? `${kind.toUpperCase()} Tracking` : `${kind.toUpperCase()} Approval`);
      ...
      <div className="mx-auto flex max-w-2xl flex-col gap-6">
        ...
        {request.approvalRequestId && (
          <ApprovalStatusPanel
            ...
            decideHrefOverride={decideHrefBuilder?.(request.approvalRequestId)}
          />
        )}
