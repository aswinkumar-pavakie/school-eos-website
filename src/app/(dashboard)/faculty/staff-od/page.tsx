...
// unchanged, zero new backend, matching the plan's screen->domain mapping.
    ...
    return (
      <div>
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            <h1 style={{ margin: 0, font: "700 36px/1.1 var(--fac-font-sans)", letterSpacing: "-.02em" }}>Staff OD</h1>
            <p style={{ margin: "8px 0 0", font: "400 15px/1.4 var(--fac-font-sans)", color: "var(--fac-body-muted)" }}>Apply for on-duty and track your applications</p>
          </div>
          <Tabs
            items={[
              { key: "Apply", label: "Apply", href: "/faculty/staff-od?tab=Apply" },
              { key: "History", label: "History", href: "/faculty/staff-od?tab=History" },
            ]}
            activeKey={activeTab}
          />
        </div>
        ...
        {activeTab === "Apply" ? (
          <StaffLeaveForm defaultType="ON_DUTY" />
        ) : requests.length === 0 ? (
          <div style={{ marginTop: 18 }}>
            <FacultyEmptyState message="Nothing has been submitted yet." />
          </div>
        ) : (
          <div className="flex flex-col gap-3.5" style={{ marginTop: 22 }}>
            {requests.map((r) => {
              ...
              return (
                <div key={r.id} className="fac-hover-lift" style={{ background: "var(--fac-white)", border: "1px solid var(--fac-border)", borderRadius: "var(--fac-radius-card)", padding: "20px 22px" }}>
                  ...
                  <div style={{ font: "700 19px/1.3 var(--fac-font-sans)", marginTop: 12 }}>On-duty request</div>
