...
// Correspondent Phase 6 addition -- Finance's per-subject-type detail views
// now exist under /correspondent/finance/* (same shared components Principal's
// own SUBJECT_LINKS point at), so these four real subject_object_type values
// can link out instead of falling back to the plain-payload treatment. Every
// OTHER subject type (e.g. `refund`, which never had a standalone detail page
// anywhere even in Finance's own UI) still correctly falls through to the
// `subjectHref &&` guard below rather than linking to a 404.
const SUBJECT_LINKS: Record<string, (id: string) => string> = {
  concession: (id) => `/correspondent/finance/concessions/${id}`,
  fee_structure: (id) => `/correspondent/finance/fee-structures/${id}`,
  expense: (id) => `/correspondent/finance/expenses/${id}`,
  purchase_request: (id) => `/correspondent/finance/purchase-requests/${id}`,
};
...
export default async function PrincipalRequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
      <div className="mx-auto flex max-w-2xl flex-col gap-6">
        <Link href="/correspondent/requests" className="text-xs font-bold text-text-muted hover:text-text">
          ← Back to Approvals
        </Link>
