  ...
  return (
      <div className="mx-auto max-w-[1280px]">
        ...
        <div className="mt-6 grid grid-cols-1 gap-[18px] sm:grid-cols-2 xl:grid-cols-4">
          ...
          <KpiCard eyebrow="Subjects offered" value={String(summary.subjectsCount)} detail="Active subjects" href="/correspondent/academics" />
          <KpiCard
          ...
          {/* Phase 5 addition -- real counts from each module's own existing
              overview service (see principal-dashboard.service.ts). */}
          <KpiCard
            eyebrow="Inventory"
            value={String(summary.inventoryLowStockCount)}
            detail={`Low stock · ${summary.inventoryDamagedCount} damaged`}
            href="/correspondent/inventory"
          />
          <KpiCard
            eyebrow="Maintenance"
            value={String(summary.maintenanceOpenRequestsCount)}
            detail="Open repair requests"
            href="/correspondent/maintenance"
          />
          <KpiCard
            eyebrow="Sports"
            value={String(summary.sportsUpcomingFixturesCount)}
            detail="Upcoming fixtures"
            href="/correspondent/sports"
          />
          <KpiCard
            eyebrow="Library"
            value={String(summary.libraryOverdueCount)}
            detail="Overdue books"
            href="/correspondent/library"
          />
          {/* Phase 8 addition -- real vehicle_document/driver_document expiry
              counts (see principal-dashboard.service.ts's complianceSummary). */}
          <KpiCard
            eyebrow="Compliance"
            value={String(summary.complianceOverdueCount)}
            detail={`${summary.complianceExpiringCount} expiring within 45 days`}
            href="/correspondent/compliance"
          />
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-[18px] lg:grid-cols-2">
          <div className="rounded-[16px] border border-border bg-surface p-[18px]">
            <div className="flex items-center justify-between">
              ...
              {/* Phase 7 addition -- /correspondent/audit now exists (Principal's
                  own dashboard has the identical "View audit trail" link). */}
              <Link href="/correspondent/audit" className="text-[13px] font-semibold text-primary">
                View audit trail
              </Link>
            </div>
        ...
        <div className="mt-6 rounded-[16px] border border-border bg-surface p-[18px]">
          <div className="flex items-center justify-between">
            ...
            <Link href="/correspondent/announcements" className="text-[13px] font-semibold text-primary">
              View all
            </Link>
          </div>
          {notices.length === 0 ? (
