// Correspondent -> Finance -> Payments: read-only mirror of Admin's own
// /admin/finance/payments page -- same real /payments data Principal's own
// page already shows (already grants CORRESPONDENT). No card/bank/gateway
// secrets in this payload, only a reference string (gatewayRef) and receipt
// number. Student links point at Correspondent's OWN Students module
// (/correspondent/students/[id]), never Admin's/Principal's.
...
export default async function CorrespondentFinancePaymentsPage({
  searchParams,
}: {
  ...
  function hrefWith(overrides: Record<string, string | undefined>) {
    ...
    return `/correspondent/finance/payments?${next.toString()}`;
  }
  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      ...
      <FinanceTabBar active="Payments" basePath="/correspondent/finance" />

      <PaymentsFilterBar
        search={params.search ?? ""}
        state={params.state ?? ""}
        mode={params.mode ?? ""}
        formActionOverride="/correspondent/finance/payments"
      />
            ...
            {payments.map((p) => (
              <tr key={p.id} className="card-hover">
                <td className="px-4 py-3 font-semibold text-text">
                  {p.studentId ? (
                    <Link href={`/correspondent/students/${p.studentId}`} className="hover:underline">
