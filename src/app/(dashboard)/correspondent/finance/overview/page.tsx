// Correspondent -> Finance -> Overview: read-only mirror of Admin's own
// /admin/finance/overview page (Fee Overview + Fee Status) -- same real
// fee-overview/fee-demands data Principal's own page already shows (both
// already grant CORRESPONDENT). Student links point at Correspondent's OWN
// Students module (/correspondent/students/[id]), never Admin's/Principal's --
// Correspondent never reuses another role's route.
...
export default async function CorrespondentFinanceOverviewPage({
  searchParams,
}: {
  ...
  function hrefWith(overrides: Record<string, string | undefined>) {
    ...
    return `/correspondent/finance/overview?${next.toString()}`;
  }
  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-text-muted">
            Fee collection visibility — view-only. Collecting payments, refunds, adjustments and reconciliation are
            Finance/Accounts operations, not part of this view.
          </p>
        </div>
      </div>
      <FinanceTabBar active="Overview" basePath="/correspondent/finance" />
      ...
      <FeeStatusFilterBar
        ...
        grades={grades}
        sections={sections}
        formActionOverride="/correspondent/finance/overview"
      />
            ...
            {demands.map((d) => (
              <tr key={d.id} className="card-hover">
                <td className="px-4 py-3 font-semibold text-text">
                  <Link href={`/correspondent/students/${d.studentId}`} className="hover:underline">
