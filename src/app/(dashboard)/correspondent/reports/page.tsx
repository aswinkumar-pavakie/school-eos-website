// Correspondent -> Reports & Analytics: same institution-wide, read-only
// cross-cutting aggregation Admin's/Principal's own Reports page shows,
// reusing getReportsSummary() and every chart component verbatim (same real
// GET /admin/reports-summary endpoint, already grants CORRESPONDENT -- see
// reports.controller.ts's own comment). This page has no write UI anywhere
// (no forms, no per-item drill-down into another role's route), so nothing
// needed stripping down, only the top-of-page Download control is omitted (no
// Principal/Correspondent page in this build exposes Download, matching
...
import {
  ReportDonutChart,
  type DonutSlice,
} from "@/components/reports/ReportDonutChart";
...
function SubLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="mb-2 text-[11px] font-bold uppercase leading-[14px] tracking-[0.06em] text-text-muted">
      {children}
    </p>
  );
}

export default async function CorrespondentReportsPage() {
    ...
    return (
      <div className="rounded-[16px] border border-border bg-surface p-8 text-center">
        <p className="text-[15px] font-extrabold leading-[20px] text-text">
          Couldn&apos;t load Reports &amp; Analytics
        </p>
  ...
  const gradeBarData = data.enrollment.byGrade.map((g) => ({
    label: g.gradeName,
    value: g.count,
  }));
  const genderDonutData = assignCategoricalColors(
    data.enrollment.byGender.map((g) => ({
      label: titleCase(g.gender),
      value: g.count,
    })),
  );

  const designationBarData = data.staff.byDesignation.map((d) => ({
    label: d.designation,
    value: d.count,
  }));

  const attendanceLineData = data.attendance.dailyPercentPresent.map((a) => ({
    date: a.date,
    value: a.percentPresent,
  }));
  ...
  const ridershipBarData = data.transport.ridershipByRoute.map((r) => ({
    label: r.routeName,
    value: r.count,
  }));
  const vehicleDonutData = buildStatusDonut(
    data.transport.vehiclesByStatus.map((v) => ({
      count: v.count,
      status: v.status,
    })),
  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-text-muted">
            Institution-wide figures, straight from each module&apos;s own real
            data.
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            <KpiCard
              ...
              value={String(data.enrollment.activeCount)}
              detail="Currently enrolled"
              icon={<StudentsIcon className="h-5 w-5" />}
            />
            <KpiCard
              eyebrow="Inactive / archived"
              value={String(data.enrollment.inactiveCount)}
              detail="Left, TC issued or archived"
              icon={<StudentsIcon className="h-5 w-5" />}
            />
          </div>
          <div className="mt-5">
            ...
            {gradeBarData.length === 0 ? (
              <EmptySection label="No active enrolment for the current academic year yet." />
            ) : (
              <ReportBarChart data={gradeBarData} />
            )}
          </div>
          <div className="mt-5">
            <SubLabel>Students by gender</SubLabel>
            {genderDonutData.length === 0 ? (
              <EmptySection label="No gender recorded on active students yet." />
            ) : (
              <ReportDonutChart data={genderDonutData} />
            )}
          </div>
        </SectionCard>
        ...
        <SectionCard title="Staff" icon={<FacultyIcon className="h-4 w-4" />}>
          <div className="grid grid-cols-2 gap-3">
            <KpiCard
              ...
              value={String(data.staff.teachingCount)}
              detail="Active, is_teaching"
              icon={<FacultyIcon className="h-5 w-5" />}
            />
            <KpiCard
              eyebrow="Non-teaching staff"
              value={String(data.staff.nonTeachingCount)}
              detail="Active, support roles"
              icon={<FacultyIcon className="h-5 w-5" />}
            />
          </div>
          <div className="mt-5">
            <SubLabel>Staff by designation</SubLabel>
            {designationBarData.length === 0 ? (
              <EmptySection label="No designation recorded on active staff yet." />
            ) : (
              <ReportBarChart data={designationBarData} />
            )}
          </div>
        </SectionCard>
        ...
        <SectionCard
          title="Attendance"
          icon={<AcademicsIcon className="h-4 w-4" />}
        >
        ...
        <SectionCard title="Fees" icon={<FinanceIcon className="h-4 w-4" />}>
          ...
          <div className="mt-5">
            ...
            {feesDonutData.length === 0 ? (
              <EmptySection label="No fee demands raised yet." />
            ) : (
              <ReportDonutChart data={feesDonutData} />
            )}
          </div>
        </SectionCard>
        ...
        >
          <div>
            ...
            {ridershipBarData.length === 0 ? (
              <EmptySection label="No active routes with student allocations yet." />
            ) : (
              <ReportBarChart data={ridershipBarData} />
            )}
          </div>
          <div className="mt-5">
            <SubLabel>Vehicle operational status</SubLabel>
            {vehicleDonutData.length === 0 ? (
              <EmptySection label="No vehicles registered yet." />
            ) : (
              <ReportDonutChart data={vehicleDonutData} />
            )}
          </div>
        </SectionCard>
        ...
        >
          ...
          {inventoryDonutData.length === 0 ? (
            <EmptySection label="No inventory items recorded yet." />
          ) : (
            <ReportDonutChart data={inventoryDonutData} />
          )}
        </SectionCard>
        ...
        >
          ...
          <div className="mt-5">
            ...
            {libraryDonutData.length === 0 ? (
              <EmptySection label="No book copies catalogued yet." />
            ) : (
              <ReportDonutChart data={libraryDonutData} />
            )}
          </div>
        </SectionCard>
