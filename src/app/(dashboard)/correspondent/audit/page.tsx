// Correspondent -> Audit Log: same general, real audit trail Admin's/
// Principal's own /audit page reads (GET /audit-log, already grants
// CORRESPONDENT -- see audit-log.controller.ts), reused verbatim -- this page
// has no write UI anywhere (pure read/filter/paginate; audit is immutable),
// so nothing needed stripping down. Confirmed no audit.record() call anywhere
// in this codebase embeds a password/token/secret in beforeData/afterData
// (checked persons.service.ts/identity.service.ts/staff.service.ts's own
// password-touching call sites), so the raw before/after JSON this page shows
// is safe as-is. The `returnTo` guard (only trust an internal, root-relative
// path) stays as-is; Correspondent's own item-detail pages pass their own
// /correspondent/* return paths.
...
export default async function CorrespondentAuditPage({
  searchParams,
}: {
  ...
  function hrefWith(overrides: Record<string, string | undefined>) {
    ...
    return `/correspondent/audit?${next.toString()}`;
  }
  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      ...
      <form action="/correspondent/audit" className="mt-6 flex flex-wrap items-end gap-3">
