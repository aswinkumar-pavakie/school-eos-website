...
// Inventory detail page (/correspondent/inventory/items/[id], built in Phase 13)
// rather than Admin's -- Principal never reuses another role's route.
// "View history" now links to Correspondent's own /correspondent/audit (Phase
// 7) -- same real objectType/objectId-scoped Audit Log drill-down pattern
// Principal's own maintenance request detail page already uses.
...
export default async function PrincipalRepairRequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-[820px]">
      <BackLink href="/correspondent/maintenance" label="Back to Repair & Maintenance" />
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href={`/correspondent/audit?objectType=repair_request&objectId=${request.id}&returnTo=${encodeURIComponent(`/correspondent/maintenance/${request.id}`)}`}
      ...
      {request.inventoryItemId && (
        <p className="mt-2 text-sm text-text-muted">
          Affected item:{" "}
          <Link href={`/correspondent/inventory/items/${request.inventoryItemId}`} className="font-semibold text-primary">
