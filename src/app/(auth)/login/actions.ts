...
// (Phase 1: login only), Transport Manager, Faculty, Parent, Hostel Warden --
// the web logins in the system. Vice Principal moved to mobile-only per
// updated plan (2026-09-03) -- see MOBILE_ALLOWED_ROLES in
// school-eos-mobile/src/lib/auth.ts. Faculty was mobile-only too until the
// product decision on 2026-09-08 to bring the same module to the web (see
// src/app/(dashboard)/faculty); Parent got the same treatment right after
// (see src/app/(dashboard)/parent). FACULTY also covers Sports Faculty
// (Sports In-Charge) operations (see /sports's own layout.tsx) -- the
// /sports module only ever shows data for sport(s) a SPORTS_FACULTY
